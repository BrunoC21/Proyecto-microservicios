...
1.1 Uso de Docker Compose:
En orden de poder realizar o ejecutar el proceso de pruebas, es necesario utilizar Docker Compose para construir el sistema de microservicios, para esto es necesario acceder a la carpeta del proyecto tras descomprimirlo y ejecutar el siguiente codigo "docker compose up -d --build."

Nota: Es posible que alguno de los microservicios se apaguen inicialmente previo a su ejecución, esto es solucionable tras la ejecución de: "docker restart <nombre del microservicio>"

Tras su uso se puede eliminar los containers respectivos mediante el comando "docker compose down"
...
2. Procesos de prueba Postman:
   ...
   a. Para insertar una nueva evaluación, primero acceda a Postman e inserte el siguiente link: "http://localhost:5001/evaluaciones".
   b. Luego coloca el proceso a la izquierda del link, el metodo POST.
   c. Selecciona la opción BODY, dale click a "raw" y configuralo para texto en formato JSON.
   d. Inserta el código en el siguiente formato, llenalo con los datos que te gusten y haz click en SEND:
   {
   ...
   a. Primero acceda a Postman e inserte el siguiente link: "http://localhost:5001/evaluaciones/<rut_estudiante>". En donde el rut debe estar en el formato "11111111-1".
   b. Luego coloca el proceso a la izquierda del link, el metodo GET.
   c. Haga click en SEND, como resultado deberia entregar la evaluación asociada.
   d. Si no existe la evaluación entregará el mensaje: "{"mensaje": "Evaluación no encontrada"}".
3. Procesos de prueba base de datos:
   Para realizar estas pruebas es necesario usar el comando "docker exec -it db psql -U user -d microservicios" en la consola de comandos, esto permite revisar la base de datos de manera directa
   3.1. Estudiantes:
   3.1.1 Insertar estudiantes:
   a. Para insertar un nuevo estudiante, utilice el comando de postgress con un formato similar a:
   "INSERT INTO student (rut, nombre*completo, edad, curso) VALUES ('12345678-9', 'Ana Pérez', 21, 'Ingeniería Civil');"
   ...
   a. Para esto solo inserte como ejemplo: "SELECT _ FROM student WHERE rut='12345678-9';"
   ...
   "ERROR: insert or update on table "evaluacion" violates foreign key constraint "evaluacion_rut_estudiante_fkey" DETAIL: Key (rut_estudiante)=(22345678-9) is not present in table "student"."
   3.2.2 Entregar todos las evaluaciones:
   a. Para revisar las evaluaciones solo inserte "SELECT _ FROM evaluacion;"
   b. Este entregará una tabla con las evaluaciones que esten presentes en el momento.
   3.2.3 Entregar un evaluación por RUT:
   a. Para esto solo inserte como ejemplo: "SELECT \_ FROM evaluacion WHERE rut_estudiante='12345678-9';"
   b. Si esto es exitoso entregará una tabla con las evaluaciones que posea el rut correspondiente, en caso de que no exista no mostrará nada en la tabla.
